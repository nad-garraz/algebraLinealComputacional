...
\begin{enumerate}[label=\alph*)]
  \item
        El grafo que describe el proceso de Markov para $k \leq n$ estados:
        $$
          \begin{tikzpicture}[
            node distance=2cm,
          ...
          \end{tikzpicture}
        $$
        Cuya matriz $M \en \reales^{n \times n}$:
        $$
          M =
          \matriz{c|c}{
            \begin{array}{c|c}
              \bm{0^t} & 1      \\\hline
              I_{k-1}  & \bm{0}
            \end{array} & A_{(n-k)} \\ \hline
            \bm{0}_{(n-k)} & B_{k}
          }
        $$
        Con el bloque de arriba a la izquierda, lo llamo $C$:
        $$
          C = \begin{pmatrix}
            0      & 0      & \cdots & 0      & 1      \\
            1      & 0      & \cdots & 0      & 0      \\
        ...
        $$
          ...
          \Sii{desarrollando}[por $F_1 \ytext \llamada1$]
          \lambda^k - 1 = 0
          \sii
          \cajaResultado{
            \lambda \en G_k
        ...
        $$
        \ul{Todos los elementos de $G_{\blue{k}}$ tienen módulo 1}, son todos distintos. El autovalor $\lambda = 1$ está en el grupo.

        El autovector de $M$, $v_{\lambda = 1} \en \reales^n$ asociado al autovalor 1:
        $$
          v_{\lambda = 1} = (\ob{1, \cdots, 1,}{k \text{ elementos}} 0, \cdots, 0)
        $$
        Es el estado en que cada nodo le pasa uno, pero es un grafo circular por lo que el estado no cambia.
        ...
        \footnotesize
        \parrafoDestacado{
          $\llamada1$  Este resultado se da para $k$ par, si $k$ es impar quedan las raíces rotadas, porque el polinomio característico queda
          $$
            \lambda^k + 1 = 0
          $$
        }
