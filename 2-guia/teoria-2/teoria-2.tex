\tikzset{
  plot settings/.style={
      scale=1,
      baseline=0,
      grid/.style={ultra thin, gray!30},
      flecha/.style={-latex}
    },
  add axes/.code={
      \draw[grid] (-1.5,-1.5) grid[step=0.25] (1.5,1.5);
      \draw[flecha] (-1.5,0) -- (1.5,0) node[below]{$x_1$};
      \draw[flecha] (0,-1.5) -- (0,1.5) node[left]{$x_2$};
    }
}
...
\begin{enumerate}[label=\tiny\purple{\faIcon{snowman}}]
  ...
  \item Si $A \en \reales^{n \times n}$ es inversible, $b, \Delta b \en \reales^{n}, Ax = b \ytext A(x + \Delta x) = b + \Delta b$ entonces,
        ...
        Dada una matriz $A \en K^{n \times n}$ y una norma vectoria $\normaBullet_n$ en $K^n$
        respectivamente, definimos la norma en el espacio de matrices de $n \times n$ como:
        $$
          \norma{A}_n =
          \maximo_{\scriptscriptstyle 0 \distinto x \en K^n} \frac{ \norma{Ax}_n}{\norma{x}_n} =
          \maximo_{\scriptscriptstyle x \en K^n, \norma{x}_n = 1} \norma{Ax}_n.
        $$
...
$$
  \begin{tikzpicture}[
      ...
      guias/.style={dotted, gray, thick},
    ]
    \draw[flecha={gray}] (-1.7,0) -- (1.7,0) node[below] {$x$};
    \draw[flecha={gray}] (0,-1.7) -- (0,1.7) node[left] {$y$};

    \draw[flecha={blue}, thick] (0,0) -- (1.5,0);
    \draw[flecha={blue}, thick] (0,0) -- (0,1.5);

    \draw[flecha={red}, thick] (0,0) -- ({1.5*cos(30)},{1.5*sin(30)}) node[midway, right] {$u$};
    \draw[guias] (0,{1.5*sin(30)}) node[left] {$u_{y\theta}$} -- ({1.5*cos(30)},{1.5*sin(30)}) -- ({1.5*cos(30)},0) node[below] {$u_{x\theta}$};

    \draw[flecha={red}, thick] (0,0) -- ({-1.5*sin(30)},{1.5*cos(30)}) node[midway, left] {$v$};
    \draw[guias] (0,{1.5*cos(30)}) node[right] {$v_{y\theta}$} -- ({-1.5*sin(30)},{1.5*cos(30)}) -- ({-1.5*sin(30)},0) node[below] {$v_{x\theta}$};
